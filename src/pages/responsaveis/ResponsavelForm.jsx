...
const ResponsavelForm = (props) => {
  ...
  useEffect(() => {
    ...
    if(id){
      ResponsavelService.get(id).then(results=>{
  ...
  function enviarDados(data) {
    ...
    const resultado = dados.id ? ResponsavelService.update(dados.id, data) : ResponsavelService.create(data)
    ...
    <Menu title="Paciente">
      ...
      <Form>
        ...
        <div className="text-center mt-3 mb-3">
          ...
          <Link to="/responsaveis" className="btn btn-danger ml-1">Voltar</Link>
        </div>
      </Form>
    </Menu>
  )
...
export default withRouter(ResponsavelForm)